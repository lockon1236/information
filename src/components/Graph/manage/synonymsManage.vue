<template>
  <div id="keyManage">
    <a-row style="margin-top: 1%;">
      <a-col :span="16">
        <div style="font-size:40px;">同义词管理</div>
      </a-col>
      <a-col :span="6">
    ...
    <a-row style="margin-top: 1%;">
      <a-col :span="6" class="syRightPanel">
        <a-row style="margin-top: 1%;">
        ...
        <a-row style="margin-top: 2vh;">
          ...
          >
            <a-icon slot="smile" type="copy" />
          </a-tree>
        </a-row>
        <!-- <a-row style="margin-top: 2vh;">
          <a-button type="primary" @click="showTypePanel">添加类别</a-button>
      ...
      <a-col :span="17">
        <div class="syLeftPanel">
          <a-popover title="请选择操作" trigger="click" :key="index" v-for="(item,index) in keyList">
            <a @click="editKey" slot="content" style="margin-left: 20px;">编辑</a>
            <a @click="removeKey" slot="content" style="float: right;margin-right: 20px;">删除</a>
            <a-button
              name="keyBtn"
              class="keyBtn"
              type="primary"
    ...
    <!-- 增加同义词面板 -->
    <a-modal title="新增同义词" v-model="addState" :mask="false" :width="400" :footer="null">
      <a-row>
        <a-col :span="2"></a-col>
        <a-col :span="6">
          <div>同义词名:</div>
        ...
        <a-col :span="14">
          <a-input placeholder="请输入同义词" :value="addName" @change="addNameChange" />
        </a-col>
        <a-col :span="2"></a-col>
      </a-row>
      <a-row style="margin-top: 20px">
        <a-col :span="2"></a-col>
        <a-col :span="10">
          <a-button type="primary" @click="addToDatabase">确定</a-button>
        </a-col>
        <a-col :span="10">
          <a-button style="float:right" type="primary" @click="hideAdd">取消</a-button>
        </a-col>
        <a-col :span="2"></a-col>
      </a-row>
    </a-modal>
    <!-- 编辑同义词面板 -->
    <a-modal title="编辑同义词" v-model="editState" :mask="false" :width="500" :footer="null">
      <a-row>
        <a-col :span="2"></a-col>
        <a-col :span="6">
          <div>同义词名:</div>
        ...
        <a-col :span="14">
          <a-input placeholder="请输入同义词" :value="editName" @change="editNameChange" />
        </a-col>
        <a-col :span="2"></a-col>
      </a-row>
      ...
      <a-row style="margin-top: 20px">
        <a-col :span="2"></a-col>
        <a-col :span="10">
          <a-button type="primary" @click="editToDatabase">确定</a-button>
        </a-col>
        <a-col :span="10">
          <a-button style="float:right" type="primary" @click="hideEdit">取消</a-button>
        </a-col>
        <a-col :span="2"></a-col>
      </a-row>
    </a-modal>
    <!-- 删除面板 -->
    <a-modal title="删除同义词" v-model="removeState" :mask="false" :width="400" :footer="null">
      <a-row>
        <a-col :span="2"></a-col>
        <a-col :span="20">
          <div>{{removeTitle}}</div>
      ...
      <a-row style="margin-top: 20px">
        <a-col :span="2"></a-col>
        <a-col :span="10">
          <a-button type="primary" @click="removeToDatabase">确定</a-button>
        </a-col>
        <a-col :span="10">
          <a-button style="float:right" type="primary" @click="hideDelete">取消</a-button>
        </a-col>
        <a-col :span="2"></a-col>
      </a-row>
    </a-modal>
    <!-- 添加类别面板 -->
    <a-modal title="添加类别" v-model="typeState" :mask="false" :width="400" :footer="null">
      <a-row>
        <a-col :span="2"></a-col>
        <a-col :span="20">
          <div>{{removeTitle}}</div>
...
export default {
  ...
  methods: {
    ...
    onSearch(value) {
      ...
      fetch("http://" + window.server + "/search_word_in_synonyms/", {
        method: "POST",
    ...
    getDataList(type1, type2) {
      ...
      fetch("http://" + window.server + "/get_synonyms_list_by_type/", {
        method: "POST",
        body: JSON.stringify(formData)
      })
        .then(res => res.json())
    ...
    addToDatabase() {
      var formData = {
        ...
        synonyms: this.addName
      };
      fetch("http://" + window.server + "/add_synonyms_word/", {
        method: "POST",
        body: JSON.stringify(formData)
      })
        .then(res => res.json())
    ...
    removeKey() {
      this.removeTitle = `确定删除同义词:${this.editName}?`;
    ...
    removeToDatabase() {
      ...
      fetch("http://" + window.server + "/del_synonyms_word/", {
        method: "POST",
        body: JSON.stringify(formData)
      })
        .then(res => res.json())
    ...
    editToDatabase() {
      ...
      this.types.map(type => {
        type.children.map(child => {
          if (
            type.value == this.cascaderVal[0] &&
            child.value == this.cascaderVal[1]
          ) {
            newType = child.label;
      ...
      fetch("http://" + window.server + "/update_synonyms_word/", {
        method: "POST",
        body: JSON.stringify(formData)
      })
        .then(res => res.json())
